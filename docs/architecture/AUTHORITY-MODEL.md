...
| Human decision      | World Human-in-the-Loop / IDKit      | A unique human made a decision bound to this action before expiry             | Employment, company authority, or correctness of the action   |
...
subject, role, and action. The demo must show:
...
AND every counted delegate is enrolled and currently human-backed
AND required AgentBook-distinct and action-scoped-distinct quorums are present
AND every counted decision has a valid required company role
AND every World proof, approval session, and agent challenge was atomically consumed once
AND the configured verification envelope is valid and MATCH
AND verification, human decisions, and agent execution bind the same digest and policy
